...
export const markMessagesAsRead = createAsyncThunk(
  ...
  async ({ senderId, receiverId }, { rejectWithValue, dispatch }) => {
    try {
      ...
      dispatch(chatSlice.actions.resetUnreadForPartner({ partnerId: senderId }));
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentChat: (state, action) => {
      ...
      state.loading = true; // To trigger fetching history
        // Also reset per-partner count when chat becomes active
    const loggedInUserId = action.meta?.loggedInUserId; // Assuming you can pass this or get from auth state
    const partnerId = state.currentChat.tutorId === loggedInUserId ? state.currentChat.learnerId : state.currentChat.tutorId;
    if (partnerId && state.unreadCountsByPartner[partnerId]) {
      state.unreadCountsByPartner[partnerId] = 0;
    }
    },
  ...
  incrementUnreadForPartner: (state, action) => {
    const { partnerId } = action.payload;
    if (!state.unreadCountsByPartner) {
        state.unreadCountsByPartner = {};
    }
    if (state.unreadCountsByPartner[partnerId]) {
      state.unreadCountsByPartner[partnerId]++;
    } else {
      state.unreadCountsByPartner[partnerId] = 1;
    }
    // --- ADD THIS LOG ---
    console.log(`[ChatSlice Reducer] incrementUnreadForPartner for ${partnerId}. New count: ${state.unreadCountsByPartner[partnerId]}. Full unreadCountsByPartner:`, JSON.stringify(state.unreadCountsByPartner));
    // --- END LOG ---
  },
  resetUnreadForPartner: (state, action) => {
    const { partnerId } = action.payload;
    if (state.unreadCountsByPartner && state.unreadCountsByPartner[partnerId] !== undefined) {
      state.unreadCountsByPartner[partnerId] = 0;
    }
  }
  },
    ...
    const handleNewMessage = (message) => {
      ...
      if (message.group && message.sender) {
        ...
        if (message.sender._id !== loggedInUserId &&
          ...
          dispatch(incrementUnreadCount({ groupId: (message.group._id || message.group) }));
        }
      }
      // --- ONE-ON-ONE MESSAGE ---
        ...
        if (message.receiverId === loggedInUserId) {
            ...
            // --- >>> CORRECTED PLACEMENT FOR NON-ACTIVE CHAT UNREAD COUNT <<< ---
                ...
                dispatch(chatSlice.actions.incrementUnreadForPartner({ partnerId: message.senderId }));
            }
            // --- >>> END CORRECTION <<< ---
        } else {
